...
public class OsmException : Exception
{
    ...
    public OsmException(int statusCode, string path, string message) : base(message)
    {
...
/// <summary>OSM token endpoint returned an error (4xx). Body kept for diagnostics.</summary>
public class OsmTokenExchangeException : OsmException
{
    ...
    public OsmTokenExchangeException(int status, string errorCode, string errorDescription, string body)
        : base(status, "/oauth2/token", $"OSM token exchange failed ({status}, {errorCode}): {errorDescription}")
    {
        ErrorCode = errorCode;
